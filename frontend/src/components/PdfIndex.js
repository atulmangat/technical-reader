...
export function PdfIndex({
  ...
  console.log(
    "DEBUG_SATS currentPage,totalPages,onPageSelect,pdf,onToggle",
    currentPage,
    totalPages,
    onPageSelect,
    pdf,
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        isExpanded &&
        indexRef.current &&
        !indexRef.current.contains(event.target)
      ) {
        toggleTocExpanded();
      }
    };
    ...
    >
      ...
      >
        {outline.length > 0 ? (
          ...
          <div className="no-outline">No document contents available</div>
        )}
      </div>
    </div>
  );
